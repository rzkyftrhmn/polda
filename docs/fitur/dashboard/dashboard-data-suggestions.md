...
- Query server-side untuk grafik dan tabel agar performa baik
- Cache agregasi (misal cache per 5–15 menit) untuk metrik berat
- Gunakan eager loading (`with`) saat perlu relasi untuk tabel
- Pastikan kolom filter yang ambigu di query menggunakan prefix tabel (contoh: `users.name` saat ada join)
- Tampilkan SweetAlert untuk notifikasi sukses/gagal saat aksi pada dashboard
